...
async function fetchResultsFromAPI(
    ...
    try {
        ...
        console.log("[v0] Fetching results from API with data:", questionAndAnswer)

        const response = await fetch("/api/model/interpret-result/", {
            method: "POST",
            headers: {
        ...
        console.log("[v0] Results API response:", data)
...
export default function QuizPage() {
    ...
    const [totals, setTotals] = useState({ stress: 0, happiness: 0, cash: 15000 })
    const [questions, setQuestions] = useState<QuizQuestion[]>([])
    const [isLoadingQuestions, setIsLoadingQuestions] = useState(true)
    const [userData, setUserData] = useState<{ name: string; feelGoodValue: number; tightnessValue: number } | null>(null)
    const [showTokenDialog, setShowTokenDialog] = useState(false)
    const [apiResults, setApiResults] = useState<ApiResultResponse["data"] | null>(null)
    ...
    useEffect(() => {
        const loadQuestions = async () => {
            try {
                ...
                console.log("[v0] Questions fetched successfully:", result.data.length)
    ...
    const handleSelect = useCallback(
        (choiceIndex: number | null) => {
            ...
            const choice = choiceIndex !== null ? currentQuestion.choices[choiceIndex] : null
        ...
        [currentQuestion, timeLeft, selectedChoice, currentQuestionIndex, questions, userResults],
    )
    ...
    const fetchResults = async () => {
        try {
            ...
            console.log("[v0] Fetching results with final data:", finalResults.question_and_answer)
            const result = await fetchResultsFromServer(token, finalResults.question_and_answer)
            ...
            if (result.success && result.data && result.data.data) {
                ...
                setResultsError(result.error || "Failed to fetch results from server")
            }
        } catch (error) {
    ...
    useEffect(() => {
        if (timeLeft > 0 && selectedChoice === null && !isComplete && currentQuestion) {
    ...
    const shareResult = (platform: string) => {
        const stressPercentage = Math.min(100, Math.max(0, (totals.stress / 100) * 100))
        const happinessPercentage = Math.min(100, Math.max(0, (totals.happiness / 100) * 100))
        const cashPercentage = Math.min(100, Math.max(0, ((totals.cash - 15000) / 5000) * 100 + 50))

        const shareText = `I just completed the Jaifoo personality quiz! My results: Stress ${Math.round(stressPercentage)}%, Happiness ${Math.round(happinessPercentage)}%, Cash ${Math.round(cashPercentage)}%`
                    ...
                    <AlertDialog open={true}>
                        <AlertDialogContent>
                            <AlertDialogHeader>
                                ...
                                <AlertDialogDescription>
                                    You need to complete the personalization process before taking the quiz. This helps us provide you
                                    with personalized questions and results.
                                </AlertDialogDescription>
                            </AlertDialogHeader>
                            <AlertDialogFooter>
                                <AlertDialogCancel onClick={handleStayOnQuiz}>Stay Here</AlertDialogCancel>
                                <AlertDialogAction
                ...
                <div className="text-center">
                    {isLoadingQuestions ? (
                        ...
                        <>
                            ...
                            <p className="text-gray-600 mb-4">Failed to load questions from API</p>
                            <Button
    ...
    if (isComplete) {
        ...
        const cashPercentage = apiResults
            ? Math.min(100, Math.max(0, (((apiResults.updatedParam.cash || 15000) - 15000) / 5000) * 100 + 50))
            : Math.min(100, Math.max(0, ((totals.cash - 15000) / 5000) * 100 + 50))
                            ...
                            {apiResults && (
                                <div className="space-y-4 mb-6">
                                    {apiResults.keyinsight && (
                                        <div>
                                            <h3 className="text-sm font-semibold mb-2">Key Insight:</h3>
                                            <p className="text-sm opacity-90">{apiResults.keyinsight}</p>
                                        </div>
                                    )}
                                    {apiResults.strength && (
                                        <div>
                                            <h3 className="text-sm font-semibold mb-2">Strength:</h3>
                                            <p className="text-sm opacity-90">{apiResults.strength}</p>
                                        </div>
                                    )}
                                    {apiResults.areaOfImprovement && (
                                        <div>
                                            <h3 className="text-sm font-semibold mb-2">Area of Improvement:</h3>
                                            <p className="text-sm opacity-90">{apiResults.areaOfImprovement}</p>
                                        </div>
                                    )}
                                    {apiResults.suggestion && (
                                        <div>
                                            <h3 className="text-sm font-semibold mb-2">Suggestion:</h3>
                                            <p className="text-sm opacity-90">{apiResults.suggestion}</p>
                                        </div>
                                    )}
                                </div>
                            )}
        ...
        <div className="min-h-screen bg-white">
            ...
            <main className="flex flex-col items-center justify-center px-6 py-8 max-w-lg mx-auto">
                <div className="mb-8 text-center">
                    <div className="mb-2">
                        <span className="text-sm text-gray-500 bg-gray-100 px-3 py-1 rounded-full">{currentQuestion.category}</span>
                    </div>
                    <h1 className="text-2xl font-bold text-gray-900 text-balance">{currentQuestion.question}</h1>
                </div>
                ...
                <div className="w-full space-y-4 mb-8">
                    {currentQuestion.choices.map((choice, index) => (
                        <Button
                            ...
                            className={`w-full p-6 text-left rounded-xl border-2 transition-all ${selectedChoice === index
                ...
                <div className="w-full">
                    <div className="flex items-center gap-4">
                        ...
                        <div className="flex-1">
                            ...
                            <div className="text-sm text-gray-600 mt-1 text-center">{timeLeft}s remaining</div>
                        </div>
                    </div>
                </div>

                <div className="mt-6 text-center">
                    <p className="text-sm text-gray-500">Use keyboard shortcuts: 1, 2, 3</p>
                </div>
            </main>
        </div>
    )
