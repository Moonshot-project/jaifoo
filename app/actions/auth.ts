...
export async function authenticateUser(payload: AuthPayload): Promise<AuthResponse> {
  const API_URL = "https://api.jaifoo.app/api/user/auth/api-key/"
  ...
  try {
    ...
    if (!res.ok) {
      const msg = data?.message || data?.error || `Authentication failed (${res.status})`
      return {
        success: false,
        error: msg,
      }
    ...
    const userName = data?.data?.user_name ?? data?.user_name ?? payload.user_name
    const userId = data?.data?.user_demo_id ?? data?.data?.user_id ?? data?.user_demo_id ?? data?.user_id ?? null
